...
public class TargetScanner
{
    ...
    public float detectionRadiusWhenSpotted = 0;
    [Range(0.0f, 360.0f)]
    ...
    {
        Collider[] m_colliders = Physics.OverlapSphere(detector.position, detectionRadius, targetLayer);
        foreach (Collider m_collder in m_colliders)
        {
            if (m_collder.GetComponentInParent<HealthPoints>().curHealthPoints > 0)
                target = m_collder.GetComponentInParent<HealthPoints>().gameObject;
        }
